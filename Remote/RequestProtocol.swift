...
public protocol RequestProtocol {
    ...
    /// Defines if response could be cached for this request. False by default.
    var isCacheable: Bool { get set }
...
public extension RequestProtocol {
	func headers(in service: ServiceProtocol) -> HeadersDict {
		...
		self.headers?.forEach({ key, value in params[key] = value })
...
extension Dictionary where Key == String, Value == Any? {
    ...
    public func urlEncodedString(base: String = "") throws -> String {
        guard !self.isEmpty else { return base } // nothing to encode
